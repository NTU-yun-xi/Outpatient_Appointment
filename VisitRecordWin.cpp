...
: Window(win_startX, win_startY, win_width, win_height)
{
	...
	this->edit1 = new Edit(EDIT,39,10,20,4,"",40,0,4);
	this->edit2 = new Edit(EDIT,39,15,20,4,"",40,0,4);
...
{
	this->lab3->SetContext(this->mainApp->getPatientId());
	this->edit1->SetContext(this->mainApp->getDesc());
	this->edit2->SetContext(this->mainApp->getVisitDesc());
	Window::paintWindow();
...
void VisitRecordWin::saveVisitRecord()
{
	...
	this->mainApp->setVisitDesc(this->edit2->GetContext());
	this->mainApp->setState(3);
	string doctorId = this->mainApp->getDoctorId();
	...
	for(saveIter; saveIter != apps.end(); ++saveIter)
    {
        ...
        {
        	if(app->getDay() == this->mainApp->getDay() && app->getTime() == this->mainApp->getTime())
			{
				thisdocapps.push_back(this->mainApp);
			}
			else
			{
